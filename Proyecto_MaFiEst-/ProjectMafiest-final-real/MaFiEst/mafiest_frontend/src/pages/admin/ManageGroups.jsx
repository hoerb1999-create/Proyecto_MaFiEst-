...
const ManageGroups = () => {
  ...
  const fetchGroups = async () => {
    try {
      ...
      const response = await axios.get(
        `/api/groups?page=${pagination.page}&limit=${pagination.limit}&search=${searchTerm}`
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message || "Error al procesar el grupo"
      );
      console.error("Error:", error);
    }
  };
  ...
  const handleMemberManagement = async (groupId, userId, role, action) => {
    try {
      ...
      toast.success(
        `Usuario ${action === "add" ? "añadido" : "removido"} exitosamente`
      );
                    ...
                    <div className="members-list">
                      ...
                      >
                        ...
                        {availableUsers.teachers
                          .filter(
                            (t) => !group.docentes.find((d) => d.id === t.id)
                          )
                          .map((teacher) => (
                    ...
                    <div className="members-list">
                      ...
                      >
                        ...
                        {availableUsers.students
                          .filter(
                            (s) => !group.estudiantes.find((e) => e.id === s.id)
                          )
                          .map((student) => (
